...
namespace kironj
{
    ...
    public:
        ...
        {
            auto &callback = m_eventCallback[static_cast<size_t>(event.get_type())];
            if (callback)
            {
                callback(event);
            }
    ...
    private:
        std::array<std::function<void(BaseEvent &)>, static_cast<size_t>(EventType::EventsCount)> m_eventCallback;
    };
